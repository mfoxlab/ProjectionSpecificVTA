workdir = "/Users/hsohail/OneDrive - Penn State Health/Documents/WGCNA/PFC/iRegulon"
PFCworkdir = "/Users/hsohail/OneDrive - Penn State Health/Documents/WGCNA/PFC"
setwd(workdir)
...
PFC_gene_summary <- read.table(file.path(PFCworkdir, "PFC_gene_summary.txt"), sep = '\t', header = TRUE, quote = '')

excel_file <- "PFC iRegulon Masterlist.xlsx"
...
for (sheet in sheets) {
  ...
  # Join with the PFC_gene_summary by 'mgi_symbol'
  joinedPFC <- inner_join(cleaned_tfs_df, PFC_gene_summary, by = 'mgi_symbol')
  ...
  output_file <- paste0("PFC_M", module_number, "_TF_DE.txt")
  
  # Write the resulting data to a file
  write.table(joinedPFC, file = output_file, row.names = FALSE, sep = '\t', col.names = TRUE, quote = FALSE)
