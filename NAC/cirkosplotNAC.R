...
setwd(workdir)
load(file = paste0(workdir,"/cirkosplotNACworkspace.Rdata"))
...
module_info <- read.table("NAC_cirkosdata.txt", sep = '\t', header = TRUE, quote = '')
...
colnames(module_info_logp)[7:9] <- c("logP.Value_NAC_FENT", "logP.Value_NAC_F_FENT", "logP.Value_NAC_M_FENT")
...
circos.clear()
#pdf("cirkosplot_NAC.pdf", width = 9.5, height = 7)
...
# Load the module information and module color data
module_info <- read.table("NAC_cirkosdata_all_MEs.txt", sep = '\t', header = TRUE, quote = '')
color_data <- read.delim(file.path(colordir, "modulecolors.txt"), header = TRUE, stringsAsFactors = FALSE, quote = '', fill = TRUE)
...
circos.clear()
#pdf("cirkosplot_NAC_all_MEs.pdf", width = 9.5, height = 7)
circos.par(start.degree = -180, gap.degree=0.2)
...
# Save the workspace
save.image(file = paste0(workdir, "/cirkosplotNACworkspace.Rdata"))
